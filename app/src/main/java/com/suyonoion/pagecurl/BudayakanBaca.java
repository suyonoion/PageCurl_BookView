...
public class BudayakanBaca extends GLSurfaceView implements View.OnTouchListener,
	...
	//TAMBAHAN
	private BudayakanBaca mCurlView;
	 ...
	 * Default constructor.
	 */
	public BudayakanBaca(Context ctx) {
		super(ctx);
		init(ctx);
	}
	 ...
	 * Default constructor.
	 */
	public BudayakanBaca(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
		init(ctx);
	}
	 ...
	 * Default constructor.
	 */
	public BudayakanBaca(Context ctx, AttributeSet attrs, int defStyle) {
		this(ctx, attrs);
	}
	...
	private void init(Context ctx) {
		...
		//tambahan
		mCurlView = (BudayakanBaca) findViewById(setResource("HAJAR_HArus_belaJAR","id"));
		mCurlView.setPageProvider(new PageProvider() {

			// Bitmap resources.
			private int[] mBitmapIds = {
					setResource("sampul_depan","layout"),
					setResource("layout_kertas1","layout"),
					setResource("layout_kertas2","layout"),
					setResource("layout_kertas3","layout"),
					setResource("layout_kiri","layout"),
					setResource("sampul_belakang","layout")};
			...
			private Bitmap loadBitmap(int width, int height, int index) {
				...
				View v = inflater.inflate(mBitmapIds[index], null,true);
				v.measure(
			...
			public void updatePage(CurlPage page, int width, int height, int index) {
				...
				switch (index) {
					...
					case 0: {
						...
						Bitmap kiri = loadBitmap(width, height, 4);
						page.setTexture(front, CurlPage.SIDE_FRONT);
						page.setTexture(kiri, CurlPage.SIDE_BACK);
					...
					case 3: {
						...
						Bitmap kiri = loadBitmap(width, height, 5);
						page.setTexture(front, CurlPage.SIDE_FRONT);
						page.setTexture(kiri, CurlPage.SIDE_BACK);
						page.setColor(Color.rgb(180, 180, 180), CurlPage.SIDE_BACK);
						break;
		...
		mCurlView.setSizeChangedObserver(new SizeChangedObserver() {
			...
			public void onSizeChanged(int w, int h) {
				if (w > h) {
					mCurlView.setViewMode(BudayakanBaca.SHOW_TWO_PAGES);
					mCurlView.setMargins(.03f, .03f, .03f, .03f);
				} else {
					mCurlView.setViewMode(BudayakanBaca.SHOW_ONE_PAGE);
					mCurlView.setMargins(.03f, .03f, .03f, .03f);
				}
			}
		});
		mCurlView.setCurrentIndex(0);
		mCurlView.setBackgroundColor(0x7D7B6E);
	}
